...
export class AuthStateService {
  ...
  private loadInitialAuthState(): boolean {
    ...
    return true; // Indicate initialization attempted/completed
  }
  ...
  private persistAuthState(): void {
    // Check if running in a browser environment before accessing localStorage
    if (typeof window !== 'undefined' && window.localStorage) {
      const token = this.#authToken(); // Read current signal value
      const user = this.#currentUser(); // Read current signal value
